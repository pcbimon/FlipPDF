...
class _LazyPdfPageWidgetState extends State<LazyPdfPageWidget> with AutomaticKeepAliveClientMixin {
  ...
  String _generateCacheKey() {
    return '${widget.filePath}_page_${widget.pageNumber}_${widget.quality.name}';
  }
  ...
  Future<void> _loadPageIfNeeded() async {
    ...
    try {
      ...
      final diskCachedPage = await PdfProcessor.loadSinglePageFromDiskCache(cacheKey);
  ...
  Widget build(BuildContext context) {
    ...
    return _pageContent ?? Container(
      color: Colors.black,
      child: const Center(
        child: Text('ไม่สามารถโหลดหน้านี้ได้', style: TextStyle(color: Colors.white)),
      ),
    );
  }
}
